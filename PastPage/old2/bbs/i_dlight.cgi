...
#｜
#｜ modified for i-mode by ckuma <i_dlight.cgi>
#├─────────────────────────────────
#│ Original CGI Script
#├─────────────────────────────────
...
[<a href="$i_script?mode=find">ワード検索</a>]
EOM
#[<a href="$i_script?mode=howto">留意事項</a>]
...
#	if ($link) { $a_link = "・メッセージに含まれるＵＲＬには自動的にリンクが張られます。<br>\n"; }
	if ($passkey) { $pass_msg = "・削除キーは<b>必須</b><br>\n"; }
...
#			else { print "[<a href=\"$i_script?page=$y\">$x</a>]\n"; }
#			$x++;
#			$y += $i_plog;
#			$i -= $i_plog;
#		}
...
sub search {
	...
	print "キーワード <input type=text name=word size=10 value=\"$in{'word'}\"> ";
