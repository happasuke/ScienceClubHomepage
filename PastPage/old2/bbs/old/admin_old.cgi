...
<h3 style="font-size:12pt">記事メンテナンス</h3>
<form action="$admin" method="POST">
...
EOM
	...
	while (<IN>) {
		...
		"[<b>$no</b>] <b style='color:$subcol'>$sub</b> - <b>$nam</b> ",
		"- $dat<DD>$com <font color=\"$subcol\">&lt;$hos&gt;</font>\n";
	}
	close(IN);
...
sub setup {
	...
	print <<"EOM";
[<a href="$script?">掲示板に戻る</a>]
<h3 style="font-size:12pt">設定画面</h3>
<UL>
<LI>修正する部分のみ変更してください。
<form action="$admin" method="POST">
<input type=hidden name=pass value="$in{'pass'}">
...
EOM
	...
	"<tr><td>投稿間隔</td><td>",
	"<input type=text name=wait size=5 value=\"$wait\"> 秒 &nbsp; ",
	"（同一ホストの連続投稿制御）</td></tr>\n";

	if ($sendmail) {
		print "<tr><td colspan=2><hr></td></tr>\n",
		"<tr><td>Ｅメール</td>",
		"<td><input type=text name=mail size=30 value=\"$mail\"><br>\n",
		"（メール通知する場合）</td></tr>\n";
	}
